...
export const getDashboardData = asyncHandler(async (req, res) => {
  ...
  const dealAnalytics = await Deal.aggregate([
    { $match: { projectId: projectObjectId } },
    {
      $group: {
        ...
        totalCount: { 
          $sum: { 
            $cond: [{ $in: ['$status', ['closed-won', 'closed-lost', 'open', 'qualified', 'proposal', 'negotiation', 'on-hold']] }, 1, 0] 
          } 
        },
        wonCount: { 
          $sum: { 
  ...
  const conversionRate = totalLeads > 0 ? (leadAnalytics[0]?.qualified || 0) / totalLeads * 100 : 0;
  const winRate = totalDeals > 0 ? (dealAnalytics[0]?.wonCount || 0) / totalDeals * 100 : 0;
  const taskCompletionRate = totalTasks > 0 ? (taskAnalytics[0]?.completed || 0) / totalTasks * 100 : 0;
