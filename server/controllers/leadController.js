...
export const createLead = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    throw new ForbiddenError(
      "You do not have permission to create leads in this project"
    );
  }
...
export const getProjectLeads = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    throw new ForbiddenError(
      "You do not have permission to view leads in this project"
    );
  }

  const leads = await Lead.findByProject(projectId, {
    limit,
    skip,
    sort,
    order,
  ...
  res.json({
    ...
    data: {
      ...
      pagination: {
        ...
        skip: parseInt(skip),
        hasMore: parseInt(skip) + leads.length < total,
      },
    },
  });
});
...
export const getArchivedLeads = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    throw new ForbiddenError(
      "You do not have permission to view archived leads in this project"
    );
  }

  const leads = await Lead.findArchivedByProject(projectId, {
    limit,
    skip,
    sort,
    order,
...
export const unarchiveLead = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    throw new ForbiddenError(
      "You do not have permission to unarchive this lead"
    );
  }
  ...
  try {
    ...
    console.error("Failed to create lead unarchive notification:", error);
  }
...
export const updateLead = asyncHandler(async (req, res) => {
  ...
  if (lead.convertedAt) {
    throw new ValidationError(
      "Cannot edit a lead that has been converted to customer"
    );
  }
...
export const archiveLead = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    console.error("Failed to create lead archive notification:", error);
  }
...
export const deleteLead = asyncHandler(async (req, res) => {
  ...
  if (lead.convertedAt) {
    throw new ValidationError(
      "Cannot delete a lead that has been converted to customer. Please archive it instead."
    );
  }
  ...
  try {
    // Create a minimal lead object for notification
    const leadForNotification = { _id: leadId, name: leadName };
    await notificationService.createLeadNotification(
      "lead_deleted",
      leadForNotification,
      projectId,
  ...
  res.json({
    success: true,
    message:
      "Lead deleted permanently and all associated resources have been cleaned up",
  });
});
...
export const addLeadNote = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    throw new ForbiddenError(
      "You do not have permission to add notes to this lead"
    );
  }
  ...
  try {
    ...
    console.error("Failed to create lead note notification:", error);
  }
...
export const updateLeadNote = asyncHandler(async (req, res) => {
  ...
  if (!leadValidation.isValid)
    throw new ValidationError(leadValidation.message);
  ...
  if (
    ...
    throw new ForbiddenError(
      "You do not have permission to update notes on this lead"
    );
  }
...
export const deleteLeadNote = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    throw new ForbiddenError(
      "You do not have permission to delete notes on this lead"
    );
  }
...
export const convertLead = asyncHandler(async (req, res) => {
  ...
  if (lead.company) {
    ...
    if (!companyDoc) {
      ...
      try {
        ...
        console.error(
          "Failed to notify company creation during lead conversion:",
          e
        );
      }
    }
  ...
  try {
    ...
    console.error("Failed to create lead conversion notification:", error);
  }

  // Send email to project admins/managers about lead conversion
  try {
    const customerUrl = `${
      process.env.FRONTEND_URL || "http://localhost:3000"
    }/crm/${project._id}/customers/${customer._id}`;
    const leadName = lead.name || lead.email || "Lead";

    // Get project members with admin/manager roles
    const recipients = [];
    if (project.owner && project.owner.toString() !== req.user._id.toString()) {
      const owner = await import("../models/User.model.js").then((m) =>
        m.default.findById(project.owner).select("email name")
      );
    ...
    for (const member of project.members || []) {
      if (
        ...
        const memberUser = await import("../models/User.model.js").then((m) =>
          m.default.findById(member.user).select("email name")
        );
        if (
          memberUser &&
          memberUser.email &&
          !recipients.find(
            (r) => r._id.toString() === memberUser._id.toString()
          )
        ) {
    ...
    console.error("Failed to send lead conversion emails:", error);
  }

  res.json({
    success: true,
    message: "Lead converted successfully",
    data: { customer, company: companyDoc },
  });
});
...
export const updateLeadStatus = asyncHandler(async (req, res) => {
  ...
  if (lead.convertedAt) {
    throw new ValidationError(
      "Cannot update status of a lead that has been converted to customer"
    );
  }
  ...
  try {
    ...
    console.error("Failed to create lead status update notification:", error);
  }
...
export const assignLeadToUser = asyncHandler(async (req, res) => {
  ...
  if (assignedTo && assignedTo.toString() !== req.user._id.toString()) {
    try {
      const assignedUser = await import("../models/User.model.js").then((m) =>
        m.default.findById(assignedTo).select("email name")
      );
      if (assignedUser && assignedUser.email) {
        const leadUrl = `${
          process.env.FRONTEND_URL || "http://localhost:3000"
        }/crm/${project._id}/leads/${lead._id}`;
        const leadName = lead.name || lead.email || "Lead";
  ...
  try {
    await notificationService.createLeadNotification(
      assignedTo ? "lead_assigned" : "lead_unassigned",
      lead,
      project._id,
      req.user._id,
      [req.user._id]
...
export const getLeadStats = asyncHandler(async (req, res) => {
  ...
  const byStatus = {};
  byStatusAgg.forEach((i) => {
    byStatus[i._id || "unknown"] = i.count;
  });
  const bySource = {};
  bySourceAgg.forEach((i) => {
    bySource[i._id || "unknown"] = i.count;
  });
  const growthTrend = growthAgg.map((i) => ({
...
export const getLeadInsights = asyncHandler(async (req, res) => {
  ...
  const totalQualified = leads.filter(
    (lead) => (lead.stage || lead.status) === "qualified"
  ).length;
  const totalContacted = leads.filter(
    (lead) => (lead.stage || lead.status) === "contacted"
  ).length;
  const totalNew = leads.filter(
    (lead) => (lead.stage || lead.status) === "new"
  ).length;
  const totalDisqualified = leads.filter(
    (lead) => (lead.stage || lead.status) === "disqualified"
  ).length;
  ...
  const funnel = [
    { stage: "new", count: totalNew, conversionFromPrev: 100 },
    {
      ...
      conversionFromPrev:
        totalNew > 0 ? Math.round((totalContacted / totalNew) * 100) : 0,
    },
    {
      stage: "qualified",
      ...
      conversionFromPrev:
        totalContacted > 0
          ? Math.round((totalQualified / totalContacted) * 100)
          : 0,
    },
    {
      stage: "disqualified",
      count: totalDisqualified,
      conversionFromPrev:
        totalLeads > 0 ? Math.round((totalDisqualified / totalLeads) * 100) : 0,
    },
  ];
  ...
  for (let i = 11; i >= 0; i--) {
    ...
    const qualified = monthLeads.filter(
      (lead) => (lead.stage || lead.status) === "qualified"
    ).length;
    ...
    monthlyTrends.push({
      ...
      conversionRate:
        monthLeads.length > 0
          ? Math.round((qualified / monthLeads.length) * 100)
          : 0,
    });
  }
...
export const getLeadForecast = asyncHandler(async (req, res) => {
  ...
  if (
    ...
    throw new ForbiddenError(
      "You do not have permission to view lead forecast in this project"
    );
  }
  ...
  for (let i = 0; i < months; i++) {
    ...
    const qualifiedLeads = monthLeads.filter(
      (lead) => (lead.stage || lead.status) === "qualified"
    );
    const contactedLeads = monthLeads.filter(
      (lead) => (lead.stage || lead.status) === "contacted"
    );
    const newLeads = monthLeads.filter(
      (lead) => (lead.stage || lead.status) === "new"
    );
    ...
    monthlyData.push({
      ...
      conversionRate:
        monthLeads.length > 0
          ? (convertedLeads.length / monthLeads.length) * 100
          : 0,
      qualificationRate:
        monthLeads.length > 0
          ? (qualifiedLeads.length / monthLeads.length) * 100
          : 0,
    });
  }
  ...
  const funnelTrends = {
    newToContacted:
      monthlyData.reduce(
        (sum, month) => sum + month.contacted / Math.max(month.new, 1),
        0
      ) / months,
    contactedToQualified:
      monthlyData.reduce(
        (sum, month) => sum + month.qualified / Math.max(month.contacted, 1),
        0
      ) / months,
    qualifiedToConverted:
      monthlyData.reduce(
        (sum, month) => sum + month.converted / Math.max(month.qualified, 1),
        0
      ) / months,
  };
  ...
  const avgGrowth =
    monthlyData.length > 1
      ? monthlyData.slice(-3).reduce((sum, month, index, arr) => {
          ...
          return (
            sum +
            (month.total - arr[index - 1].total) /
              Math.max(arr[index - 1].total, 1)
          );
        }, 0) / Math.max(monthlyData.length - 1, 1)
      : 0;

  const avgConversionRate =
    monthlyData.reduce((sum, month) => sum + month.conversionRate, 0) / months;
  const avgQualificationRate =
    monthlyData.reduce((sum, month) => sum + month.qualificationRate, 0) /
    months;
  ...
  for (let i = 1; i <= 6; i++) {
    ...
    const projectedTotal = Math.max(
      0,
      Math.round(lastMonthData.total * Math.pow(1 + avgGrowth / 100, i))
    );

    const projectedConverted = Math.round(
      projectedTotal * (avgConversionRate / 100)
    );
    const projectedQualified = Math.round(
      projectedTotal * (avgQualificationRate / 100)
    );
  ...
  for (let i = 1; i <= 6; i++) {
    const projectedVelocity = Math.round(
      currentVelocity * Math.pow(1 + avgGrowth / 100, i)
    );
    velocityForecast.push({
      ...
      expectedConversions: Math.round(
        projectedVelocity * (avgConversionRate / 100)
      ),
    });
  }
  ...
  res.json({
    ...
    data: {
      ...
      insights: {
        ...
        expectedConversions:
          forecast[forecast.length - 1]?.projectedConverted || 0,
