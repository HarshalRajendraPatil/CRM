...
const Calendar = () => {
  ...
  const handleUpdateEvent = async (eventData) => {
    try {
      await dispatch(updateCalendarEvent({ projectId, id: selectedEvent._id, ...eventData })).unwrap();
  ...
  const getEventsForDate = (date) => {
    ...
    const filteredAggregatedEvents = (aggregatedEvents || []).filter(
      event => event.type !== 'custom' && !event._id?.toString().startsWith('custom_')
    );
    const allEvents = [...events, ...filteredAggregatedEvents];
    return allEvents.filter(event => {
      // Handle both Date objects and date strings
      const eventStartDate = formatDateLocal(event.startDate);
      const eventEndDate = event.endDate ? formatDateLocal(event.endDate) : eventStartDate;
            ...
            sortedDates.map(dateKey => {
              ...
              return (
                <div key={dateKey} className="p-4">
                  <div className={`flex items-center mb-3 ${isToday ? 'text-indigo-600 font-semibold' : ''}`}>
                  ...
                  <div className="space-y-2">
                    {dayEvents.map((event, index) => (
                      <div
                        key={index}
                        className={`p-3 rounded border ${getEventColor(event)} cursor-pointer hover:shadow-md transition-shadow`}
                        onClick={() => handleEventClick(event)}
                      >
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
    ...
    <CrmLayout>
              ...
              {showDatePicker && (
                <div className="absolute right-0 z-10 mt-2 bg-white rounded-lg shadow-lg border border-gray-200 p-4">
                  <div className="flex flex-col space-y-2">
                    ...
                    <input
                      type="date"
                      value={datePickerValue || formatDateLocal(currentDate)}
                      onChange={(e) => {
                        if (e.target.value) {
      ...
      {showStats && stats && hasManagerAccess && (
        <div className="bg-white rounded-lg shadow p-6 mb-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-semibold text-gray-900">Calendar Statistics</h2>
            <button
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-blue-50 rounded-lg p-4">
              ...
              <div className="text-2xl font-bold text-blue-900">{stats.stats?.totalEvents || 0}</div>
            </div>
            <div className="bg-green-50 rounded-lg p-4">
              <div className="text-sm text-green-600 font-medium">Upcoming</div>
              <div className="text-2xl font-bold text-green-900">{stats.stats?.upcomingEvents || 0}</div>
            </div>
            <div className="bg-red-50 rounded-lg p-4">
              <div className="text-sm text-red-600 font-medium">Overdue</div>
              <div className="text-2xl font-bold text-red-900">{stats.stats?.overdueEvents || 0}</div>
            </div>
            <div className="bg-purple-50 rounded-lg p-4">
              <div className="text-sm text-purple-600 font-medium">This Week</div>
              <div className="text-2xl font-bold text-purple-900">{stats.stats?.eventsThisWeek || 0}</div>
            </div>
          </div>
        </div>
      )}
      ...
      {showOverdue && overdueEvents && overdueEvents.length > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-lg shadow p-6 mb-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-semibold text-red-900">Overdue Events</h2>
            <button
          ...
          <div className="space-y-2">
            {overdueEvents.slice(0, 5).map((event, index) => (
              <div
                ...
                onClick={() => handleEventClick(event)}
              >
                <div className="font-medium text-sm text-red-900">{event.title}</div>
