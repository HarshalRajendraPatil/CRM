...
const CreateLeadSidebar = ({ isOpen, onClose, projectId }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    email: '',
    phone: '',
    status: '', // will be set when stages load
  ...
  // Set default status when stages are available and form opens
  useEffect(() => {
    if (isOpen && defaultStage && !formData.status) {
      setFormData(prev => ({ ...prev, status: defaultStage._id }));
    }
  }, [isOpen, defaultStage, formData.status]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      handleCancel(); // Reuses reset logic
    } catch (error) {
