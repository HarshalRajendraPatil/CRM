  ...
  const renderOverview = () => (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard
          title="Active Leads"
          value={(insights?.totals?.totalLeads || 0) - (insights?.totals?.totalDisqualified || 0)}
            ...
            <PieChart>
              <Pie
                data={[
                  ...
                  { name: 'Contacted', value: insights?.totals?.totalContacted || 0 },
                  { name: 'Qualified', value: insights?.totals?.totalQualified || 0 },
                  { name: 'Disqualified', value: insights?.totals?.totalDisqualified || 0 }
                ]}
                cx="50%"
                cy="50%"
                labelLine={false}
              ...
              <div className="flex-1 mx-4">
                <div className="bg-gray-200 rounded-full h-8">
                  <div
                    ...
                    style={{ width: `${(stage.count / Math.max(...insights.funnel.map(s => s.count))) * 100}%` }}
                  >
                    {stage.count}
                  </div>
                </div>
          ...
          <table className="min-w-full">
            <thead>
              <tr className="border-b">
                ...
                <th className="text-left py-3 px-4 font-medium text-gray-900">Qualified</th>
                ...
                <tr key={index} className="border-b hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      perf.conversionRate >= 50 ? 'bg-green-100 text-green-800' :
                      perf.conversionRate >= 25 ? 'bg-yellow-100 text-yellow-800' :
