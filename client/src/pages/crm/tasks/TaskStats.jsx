...
const TaskStats = ({ projectId }) => {
  ...
  const assignedToData = stats?.tasksByAssignedTo?.map(item => ({
    name: item.name,
    value: item.count
  })) || [];

  const relatedToData = stats?.tasksByRelatedTo?.map(item => ({
    name: item._id.charAt(0).toUpperCase() + item._id.slice(1),
    value: item.count
  })) || [];

  const completionTrendData = insights?.data?.tasksCompletedLast30Days?.map(item => ({
    date: item._id,
    completed: item.count
  })) || [];

  const creationTrendData = insights?.data?.tasksCreatedLast30Days?.map(item => ({
    date: item._id,
    created: item.count
  })) || [];
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-semibold text-gray-900">{stats?.overview?.totalTasks || 0}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-semibold text-gray-900">{stats?.overview?.completedTasks || 0}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-semibold text-gray-900">{stats?.overview?.pendingTasks || 0}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-semibold text-gray-900">{stats?.overview?.overdueTasks || 0}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <h3 className="text-lg font-medium text-gray-900 mb-4">Tasks by Assigned User</h3>
          <ResponsiveContainer width="100%" height={300}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Task Creation Trend */}
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <h3 className="text-lg font-medium text-gray-900 mb-4">Task Creation Trend (Last 30 Days)</h3>
          <ResponsiveContainer width="100%" height={300}>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <h3 className="text-lg font-medium text-gray-900 mb-4">Task Completion Trend (Last 30 Days)</h3>
          <ResponsiveContainer width="100%" height={300}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div className="text-center">
              <div className="text-2xl font-bold text-indigo-600">
                {stats?.overview?.averageCompletionTime ? `${stats?.overview?.averageCompletionTime.toFixed(1)}h` : 'N/A'}
              </div>
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-green-600">
                {stats?.overview?.completedTasks && stats?.overview?.totalTasks ? 
                  `${((stats?.overview?.completedTasks / stats?.overview?.totalTasks) * 100).toFixed(1)}%` : '0%'}
              </div>
