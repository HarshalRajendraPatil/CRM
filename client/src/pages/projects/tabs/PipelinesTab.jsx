...
const PipelinesTab = ({ projectId, project, hasManagerAccess }) => {
  ...
  const { pipelines, isLoading, isError, message } = useSelector(
    (state) => state.projects
  );
  
  useEffect(() => {
    // Only fetch pipelines once when component mounts and projectId is available
    if (projectId && !pipelines.length) {
      dispatch(getProjectPipelines(projectId));
    }
  }, [dispatch, projectId, pipelines.length]);
